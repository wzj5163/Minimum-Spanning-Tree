...
#setwd("/home/rstudio")
setwd("~/Documents/METIS/Minimum Spanning Tree/Codes/CryptocurrencyPortfolio")
...
#plot(ZOO_norm[,c(1,2,4,5,6)], ylab="normalized prices",xlab="",plot.type= "single", col=c(1:5))
...
png("MST_10Crypto_1.png", width=500, height=400, bg = "transparent")
...
node.size<-as.matrix(verticesdegreeall)*2
png("MST_10Crypto_2.png", width=500, height=400, bg = "transparent")
...
xptf<-zoo(AI,cri)
#dev.new()
chart.StackedBar(xptf, ylab= "weight", date.format="%m-%Y", cex.legend = 0.60, colorset=rainbow12equal, main="portfolio temporal composition gamma=1")
...
AI<-t(AI)
xptf<-zoo(AI,cri)
chart.StackedBar(xptf, ylab= "weight", date.format="%m-%Y", cex.legend = 0.60, colorset=rainbow12equal, main="portfolio temporal composition gamma=0")
...
legend("topright", c("Benchmark (CRIX)","Glasso Markowitz", "Equally Weighted", "Network Markowitz", "gamma=0.005","gamma=0.025","gamma=0.05", "gamma=0.15","gamma=0.7","gamma=1","Classical Markowitz"), col=c("black", "lightblue", "pink","darkslateblue","red","brown", "gold", "grey", "green", "blue3","darkorchid2"),  cex=0.5, lwd=2)
